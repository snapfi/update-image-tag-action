...
function insertYamlValue(folderPath, newImageTag) {
  if (!folderPath || !newImageTag) {
    console.error(
      "Please provide a folder path and a new image tag as arguments - folder path: " +
        folderPath +
        " tag: " +
        tag
    );
  ...
  try {
    ...
    if (yamlFiles.length === 0) {
      console.error(
        "No deployment.yaml or cronjob.yaml files found in the specified folder."
      );
      process.exit(1);
    }
    ...
    for (const yamlFilePath of yamlFiles) {
      ...
      if (resourceName === "deployment" || resourceName === "cronjob") {
        const spec = resourceConfig.spec.jobTemplate
          ? resourceConfig.spec.jobTemplate.spec
          : resourceConfig.spec;
        if (
          spec &&
          spec.template &&
          ...
          console.log(
            `Image tag for ${resourceName} in ${yamlFilePath} updated successfully.`
          );
        } else {
          console.error(
            `Failed to update image tag in ${yamlFilePath}. ${resourceName} configuration is invalid.`
          );
        }
      } else {
        console.error(
          `Unsupported resource type in ${yamlFilePath}: ${resourceName}`
        );
      }
    }
  } catch (error) {
